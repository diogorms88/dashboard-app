...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔧 Buscando dados de materiais da tabela registros:', { startDate, endDate, shift })

    // 1. Buscar registros da tabela registros com filtros
    console.log('📡 Montando query do Supabase...')
    ...
    if (startDate) {
      console.log(`📅 Aplicando filtro startDate: ${startDate}`)
      registrosQuery = registrosQuery.gte('data', startDate)
    }
    if (endDate) {
      console.log(`📅 Aplicando filtro endDate: ${endDate}`)
    ...
    if (registrosError) {
      console.error('❌ ERRO DETALHADO ao buscar registros:', {
    ...
    console.log('✅ Registros após filtro de turno:', registrosFiltrados?.length || 0)
    ...
    console.log('📊 Total de itens de produção encontrados:', allProductionDetails.length)
    console.log('🎨 Total de peças pintadas:', totalPecasPintadas)
    console.log('🚗 Total de choques:', totalChoques, '(incluindo', parachoquesDasPolainas, 'das Tera Polainas)')
    console.log('🔧 Total de componentes:', totalComponentes)
    console.log('📋 Tera Polaina LD:', teraPolainaLD, '| Tera Polaina LE:', teraPolainaLE)

    // 3. Buscar configuração de consumo da tabela configuracao_consumo_v2
    console.log('🔧 Buscando configuração de consumo...')
    let configuracao
    
    try {
      configuracao = await getConfiguracaoConsumo()
    } catch (configError) {
      console.error('❌ ERRO ao buscar configuração de consumo:', configError)
      return NextResponse.json({ 
    ...
    if (!configuracao) {
      console.error('❌ Erro: Configuração de consumo não encontrada')
      return NextResponse.json({ error: 'Configuração de consumo não encontrada' }, { status: 500 })
    }

    console.log('✅ Configuração carregada:', {
      especificas: configuracao.especificas.length,
      bases: Object.keys(configuracao.bases).length,
      modoTaxa: configuracao.modoTaxa
    })

    // 4. Calcular consumo de materiais usando a configuração real
    console.log('🧮 Iniciando cálculos de consumo...')
    ...
    allProductionDetails.forEach(detail => {
      ...
      if (detail.cor === 'Primer P&A') {
        ...
        if (configEspecifica) {
          ...
          if (configsSimilares.length > 0) {
            ...
            console.log(`🎯 Primer P&A - Fallback CORRIGIDO: P=${primerPAEstimado.toFixed(2)} (70% do total médio ${avgTotal.toFixed(2)}) (sem base nem verniz)`)
          } else {
            primerMl = (configuracao.geral.primer.consumo || 0) * quantidade
            baseMl = 0
            vernizMl = 0
            
            console.log(`🎯 Primer P&A - Config geral: P=${(configuracao.geral.primer.consumo || 0)} (sem base nem verniz)`)
          }
        }
      } else {
        ...
        if (configEspecifica) {
          ...
          console.log(`🎯 Config específica para ${detail.modelo} (${detail.cor}): P=${configEspecifica.primer}, B=${configEspecifica.base}, V=${configEspecifica.verniz}`)
        } else {
          ...
          if (configsSimilares.length > 0) {
            ...
            console.log(`💡 Usando média de ${configsSimilares.length} configs similares: P=${avgPrimer.toFixed(2)}, B=${avgBase.toFixed(2)}, V=${avgVerniz.toFixed(2)}`)
          } else {
            ...
            console.log(`🔄 Usando config geral para ${detail.modelo} (${detail.cor})`)
          }
        }
      }
      ...
      if (configuracao.modoTaxa === 'sobreTotal') {
        ...
        if (!baseConfigCor) {
          console.warn(`⚠️ Cor ${detail.cor} não encontrada nas configurações específicas!`)
        }
        ...
        console.log(`🧮 ${detail.modelo} (${detail.cor}) x${quantidade}:`)
        console.log(`   Primer: ${primerTotalMl.toFixed(1)}ml - ${primerDiluenteMl.toFixed(1)}ml diluente = ${primerMl.toFixed(1)}ml puro`)
        console.log(`   Base: ${baseTotalMl.toFixed(1)}ml - ${baseDiluenteMl.toFixed(1)}ml diluente = ${baseMl.toFixed(1)}ml puro`)
        console.log(`   Verniz: ${vernizTotalMl.toFixed(1)}ml total → ${vernizMl.toFixed(1)}ml puro + ${vernizDiluenteMl.toFixed(1)}ml diluente + ${catalisadorMl.toFixed(1)}ml catalisador`)
      }
      ...
      errosProcessamento++
      console.error(`❌ Erro ao processar item ${itensProcessados}:`, {
        modelo: detail.modelo,
        cor: detail.cor,
        qtd: detail.qtd,
        erro: itemError instanceof Error ? itemError.message : itemError
      })
    }
  })
  
  console.log(`📊 Processamento concluído: ${itensProcessados} itens, ${errosProcessamento} erros`)
    ...
    console.log('🧮 Consumo calculado:', {
      primerL: totalPrimerPuroL.toFixed(2),
      baseL: totalBasePuraL.toFixed(2),
      vernizL: totalVernizPuroL.toFixed(2),
      totalMaterialL: consumoTotalMaterial.toFixed(2),
      totalDiluentesL: consumoTotalDiluentes.toFixed(2)
    })
    ...
    console.log('✅ Dados de materiais processados:', {
      totalPecas: totalPecasPintadas,
      consumoTotal: consumoTotalMaterial,
      topModelos: topModelosArray.length,
      topCores: topCoresArray.length
    })
