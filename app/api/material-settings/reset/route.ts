...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 Resetando todas as configurações de materiais...')
    ...
    if (deleteError) {
      console.error('❌ Erro ao deletar configurações existentes:', deleteError)
    ...
    console.log(`✅ Configurações resetadas com sucesso. ${data?.length || 0} configurações padrão inseridas.`)
    return NextResponse.json({ 
    ...
    console.error('Erro na API de reset material-settings:', error)
    return NextResponse.json(
